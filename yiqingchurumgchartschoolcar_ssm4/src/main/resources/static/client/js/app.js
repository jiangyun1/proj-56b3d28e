...
var rootUrl = "http://192.168.31.207:8088/";
...
function saveRecord(){
    var fdata = {};
    fdata.uid = _focususer.id;
    fdata.username = _focususer.username;
    fdata.typecn=_typecn;
    fdata.table="record";
    ajaxCallback("saveJ",fdata,function (data) {
        showLoader("进校成功!",true);
    });
}
...
function  initswiper(){
    ...
    setTimeout(function (){
        ajaxCallback("listJ",{table:"notice"},function(data){
            ...
            for(var i=0;i<=len;i++){
                var obj = data[i];
                html+='<div onclick="toNotice(1);" style="height: 200px; background-image: url('+fileurl+obj.img+');background-size: 100% 100%;"  class="swiper-slide"></div>';
            }
